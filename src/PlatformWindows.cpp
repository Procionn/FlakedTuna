...
std::pair<std::vector<PLUG_HANDLE>, registryVector>
GetPluginHandles(const std::filesystem::path &path, const std::string &extension) {
    ...
    for (auto &p : std::filesystem::recursive_directory_iterator(path)) {
        if (p.path().extension() == pextension) {
            ...
            if (handle != nullptr) {
                ...
                RegFuncPtr PluginRegistryAddr =
                    (RegFuncPtr)GetProcAddress(handle, "GetPluginRegistry");
                ...
                if (PluginRegistryAddr != nullptr) {
                    ...
                    int pluginVersion{0};
                    VersionFuncPtr PluginFileVersion =
                        (VersionFuncPtr)GetProcAddress(handle, "GetPluginVersion");
